...
- **仍有极少数源站只放行特定地区/ISP 出口 IP**：这种情况下代理也救不了（Worker 出口是 Cloudflare 数据中心 IP），只能换源或改用 Docker 版。首次播放某源时若代理被拦，会自动回退浏览器直连（车机/家里宽带 IP 通常放行），并按主机记忆成功的方式（存于浏览器 localStorage），同主机后续播放不再弹「代理失败」提示；直连若失效也会自动反向回退代理。
