...
- **仍有极少数源站只放行特定地区/ISP 出口 IP**：这种情况下代理也救不了（Worker 出口是 Cloudflare 数据中心 IP），只能换源或改用 Docker 版。播放默认**直连优先**（车机走家庭宽带 IP，源站基本都放行、首帧最快）；若直连被源站拒绝（如 Referer 校验 403、WebDAV 鉴权 401），会秒级自动回退流媒体代理（注入源站 Referer/鉴权），并按主机记忆成功的方式（存于浏览器 localStorage），同主机后续播放不再白试失败路径；代理失效也会自动反向回退直连。
